...
use dns_stamp_parser::{DecodeErr, DnsStamp};

#[test]
fn parse_fail_one() {
    let stamp = "sdns://";

    assert_eq!(DnsStamp::decode(stamp), Err(DecodeErr::NotEnoughBytes))
}
...
fn parse_fail_two() {
    ...
    assert_eq!(
        ...
        Err(DecodeErr::Base64Error(DataEncodingDecodeError {
            position: 0,
...
fn decode_type() {
    let stamp = "sdns://DzA";
    assert_eq!(DnsStamp::decode(stamp), Err(DecodeErr::UnknownType(15)))
}
